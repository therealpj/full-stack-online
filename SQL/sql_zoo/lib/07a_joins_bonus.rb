...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT
       track.song, COUNT(track.song)
      FROM
        (SELECT DISTINCT album, song from tracks) as track
      GROUP BY track.song
      HAVING COUNT(track.song) > 2
  SQL
end
...
def best_value
  ...
  execute(<<-SQL)
    SELECT
      albums.title, albums.price, COUNT(tracks.album)
    FROM
      albums
    JOIN
      tracks
    ...
    GROUP BY
      albums.asin
    HAVING
      albums.price / COUNT(tracks.album) < .5
  SQL
end
...
def rock_superstars
  ...
  execute(<<-SQL)
  select albums.artist, count(albums.asin)
from albums join styles on styles.album = albums.asin
where styles.style like '%Rock' group by albums.artist order by count(albums.asin) desc
limit 1;
