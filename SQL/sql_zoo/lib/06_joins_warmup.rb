...
def glenn_close_id
  ...
  execute(<<-SQL)
    SELECT movies.id from movies inner join actors on movies.id = actors.id where actors.name = 'Glenn Close'
  SQL
end
...
def casablanca_cast
  ...
  execute(<<-SQL)
    SELECT actors.name from actors join castings on castings.actor_id = actors.id where castings.movie_id = 27;
  SQL
end
...
def alien_cast
  ...
  execute(<<-SQL)
    select actors.name from actors join castings on castings.actor_id = actors.id where castings.movie_id = (Select id from movies where title = 'Alien')
  SQL
end
