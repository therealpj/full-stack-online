...
def filter_out!(arr, &prc)
    n = arr.length - 1
    (0..n).to_a.reverse.each do |idx|
        arr.delete_at(idx) if prc.call(arr[idx])
    end
end
...
def multi_map(arr, n=1, &prc)
    newarr = arr
    (0...n).each do 
        newarr.each.with_index do |ele, idx|
            newarr[idx] = prc.call(ele)
        end
    end
    newarr
end
